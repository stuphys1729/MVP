...
class Animator():
    ...
    def __init__(self, lattice_queue, measure_freq, max_sweeps):
        ...
        self.M_line = Line2D(M_list, self.step_list, color="blue",
            label="Total Magnetisation")
        self.Ms_line = Line2D(Ms_list, self.step_list, color="red",
            label="Staggered Magnetisation")

        self.ax_array[1].set_xlim(0, max_sweeps)
        self.ax_array[1].set_xlabel("No. of Sweeps")
        self.ax_array[1].set_ylim(0, len(lattice)*len(lattice[0])+1)
        self.ax_array[1].set_ylabel("Total Magnetisation")
        self.ax_array[1].add_line(self.M_line)
        ...
        self.ax_array[2].set_ylabel("Staggered Magnetisation")
        self.ax_array[2].add_line(self.Ms_line)
    ...
    def update(self, i):
        ...
        if (self.queue.empty()):
            ...
            self.Ms_line.set_data(self.step_list, Ms_list)
